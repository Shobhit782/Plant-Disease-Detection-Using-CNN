...
MODEL = tf.keras.layers.TFSMLayer(str(model_path), call_endpoint='serving_default')
...
async def predict(
    ...
    print("Available prediction keys:", predictions.keys())
    
    # Get the first available key (usually the output tensor)
    output_key = list(predictions.keys())[0]
    class_probs = predictions[output_key]
